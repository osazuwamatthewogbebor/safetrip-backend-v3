...
function enqueueEmail(recipient, subject, data, name) {
    ...
    console.log({recipient, subject, data, name, retries: 0});
...
async function processQueue() {
    ...
    while (queue.length > 0) {
        ...
        try {
            ...
            logger.error(`Failed to send email to ${job.description}`, error.message);
            ...
            if (job.retries < MAX_RETRIES) {
                ...
                log.error(`Giving up on email to ${job.recipient} after ${MAX_RETRIES} attempts`);
            }
        }
    }
